...
public class PhoneEx02 {
	 ...
	 * @param count	저장된 개수
	 */
	public static void serach(Contact[] list, int count) {
		//이름을 입력
		scan.nextLine();
	...
	public static int delete(Contact[] list, int count) {
		...
		if (!checkCountact(list, count, name, index)) {
		...
		if (index != count) {
			...
			System.arraycopy(tmp, index+1, list, index, count- index+1);
		}
		//연락처 개수를 1감소
		list[count] = null;
	...
	public static void update(Contact[] list, int count) throws Exception {
		...
		boolean res = checkCountact(list, count, name, index);
		if (!res) {
			System.out.println("잘못 선택했습니다.");
			return;
		}
	 ...
	 */
	public static boolean checkCountact(Contact[] list, int count, String name, int index) {
	...
	/**
	 * 기능 : 연락처 리스트에ㅓㅅ 이름에 맞는 연락처를 출력하는 메소드
	 * @param list	연락처리스트
	 * @param count	저장된 연락처 개수
	 * @param name	검색할 이름
	 * @return	일치하는 연락처가 있으면 true, 없으면 false;
	 */
	 ...
	 * @param list	연락처리스트
	 * @param count	저장된 개수
	 * @return	저장된 개수(추가에 성공하면count+1, 아니면 count
	...
	public static int insert(Contact[] list, int count) throws Exception {
		...
		//연락처 리스트에 입력한 번호 객체가 있는지 없는지 확인
...
class Contact{
	...
	public void setNumber(String number) throws Exception {  //런타임이 아닌경우 예외 를 메소드에 적어야함
		//저어진 번호가 전화 번호 형태가 아니면 예외를 발생 시키고 맞으면 번호에 저장
		//010-1234-5677 또는 02-123-4567 형태의 문자열을 처리하기 위한 정규표현식
