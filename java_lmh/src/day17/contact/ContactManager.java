...
public class ContactManager implements Program{
	...
	public void runMenu(int menu) throws Exception {
		...
		case EXIT:
			...
			break;
		default:
			System.out.println("잘못된 메뉴입니다. 올바은 메뉴를 선택하세요.");
		}
	}
	...
	private void update() {
		...
		List<Contact>tmps = list.stream().filter(a->a.getName().contains(name))
		...
		//list에서 tmp를 제거한 리스트(원본은 그대로이어야함)B를 가져옴
		List<Contact>copylist = 
				list.stream().filter(c->!c.equals(tmp))
					.collect(Collectors.toList());
		//리스트B에 있는 연락처 중에서 입력한 번호와 같은 번호가 있는지 확인해서 
		long count =
				copylist.stream().filter(c->c.getNumber().equals(newnumber))
					.count();
		//있으면 안내문구 출력 후 종료
		if (count != 0) {
	...
	private void search() {
		...
		for (Contact tmp : list) {
			if (tmp.getName().contains(name)) {
				System.out.println(count+"번. "+tmp.getName()+" : "+tmp.getNumber());
			}
			else {
				samecount++;
			}
