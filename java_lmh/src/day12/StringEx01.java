...
public class StringEx01 {
	...
	public static void main(String[] args) {
		...
		//int indexOf(문자열) : 문자열에서 시작 번지를 반환, 없으면 -1
		System.out.println(str+"에서 world의 시작 번지는? "+ str.indexOf("world"));
		//boolean contains(String) : 문자열이 있는지 없는지 반환
		System.out.println(str+"에서 world가 있습니까? "+ str.contains("world"));
		//String replace(찾을 문자열, 교체할 문자열) : 문자열에서 찾을 문자열을 찾아서 교체할 문자열로 바꾼 후 반환
		System.out.println("Hello world에서 o를 찾아서 O로 수정 :"+ str.replace("o", "O"));
		System.out.println("수정 후"+ str);
		
		//String substring(int index) : index 번지부터 부분 문자열을 추출
		//String substring(시작 번지, 끝 번지): 시작번지부터 끝번지까지 부분 문자열을 추출(끝 번지 포함X)
		
		System.out.println(str+ " 에서 6번지부터 부분 문자열 : " + str.substring(6));
		System.out.println(str+ " 에서 6번지부터 10번지 전(9)까지 부분 문자열 : " + str.substring(6,10));
