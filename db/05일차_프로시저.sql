...
USE CGV;
# 예매자 아이디, 예매 상영시간표, 성인수, 청소년수가 주어지면 티켓을 예매하는 프로시저를 생성
drop procedure if exists TICKETTING;
DELIMITER //
CREATE PROCEDURE TICKETTING(
	IN _ID VARCHAR(15),
    IN _SD_NUM INT,
    IN _ADULT INT,
    IN _TEENAGER INT
...
BEGIN
    ...
    CASE _MORNING
		WHEN 1 THEN
			SET _ADULT_PRICE = (SELECT PR_PRICE FROM PRICE WHERE PR_TYPE = '조조 성인');
            SET _TEENAGER_PRICE = (SELECT PR_PRICE FROM PRICE WHERE PR_TYPE = '조조 청소년');
        ELSE
			SET _ADULT_PRICE = (SELECT PR_PRICE FROM PRICE WHERE PR_TYPE = '성인');
            SET _TEENAGER_PRICE = (SELECT PR_PRICE FROM PRICE WHERE PR_TYPE = '청소년');
	END CASE;
    INSERT INTO TICKETTING 
    VALUES(NULL, _ADULT, _TEENAGER, _ADULT_PRICE + _TEENAGER_PRICE, _ID, _SD_NUM);
END //
DELIMITER ;

CALL TICKETTING('abc123', 3, 3, 0);