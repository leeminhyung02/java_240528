...
SELECT ROW_NUMBER() OVER(ORDER BY PO_VIEW DESC), PO_VIEW FROM POST
WHERE  'ROWNUM' <= 3;

SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY post.po_view DESC) AS ROWNUM, post.po_view from post) AS PO_VIEW
WHERE ROWNUM <=3;
# 게시글을 조회(조회수가 많은 순으로 조회, 조회순으로 했을 때 3위 게시글까지 조회, RANK()를 이용) 
# 윈도우 함수를 WHERE에 직접 사용할 수 없어서 검색 결과를 하나의 테이블처럼 처리해야 함
SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY PO_VIEW DESC) AS RANKING, POST.* FROM POST) AS PO
WHERE  RANKING <= 3;